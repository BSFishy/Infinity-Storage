...
import infinitystorage.container.ContainerCraftingMonitor;
import infinitystorage.gui.sidebutton.SideButtonRedstoneMode;
import infinitystorage.network.MessageCraftingMonitorCancel;
...
public class GuiCraftingMonitor extends GuiBase {
    ...
    public void init(int x, int y) {
        ...
        String cancelAll = t("misc.infinitystorage:cancel_all");
    ...
    public void drawForeground(int mouseX, int mouseY) {
        ...
        for (int i = 0; i < VISIBLE_ROWS; ++i) {
            if (item < getTasks().size()) {
                ...
                if(task.getDepth() > 0){
                    x += ((float) (task.getChildren() - task.getDepth()) / (float) task.getChildren() * 16);
                }
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        if (button == cancelButton && itemSelected != -1) {
            ...
            InfinityStorage.INSTANCE.network.sendToServer(new MessageCraftingMonitorCancel(craftingMonitor, task.getId(), task.getDepth()));
        } else if (button == cancelAllButton && getTasks().size() > 0) {
            InfinityStorage.INSTANCE.network.sendToServer(new MessageCraftingMonitorCancel(craftingMonitor, -1, 0));
        }
    }
