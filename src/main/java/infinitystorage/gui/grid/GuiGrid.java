package infinitystorage.gui.grid;
...
import infinitystorage.gui.Scrollbar;
import infinitystorage.gui.grid.sorting.GridSortingName;
import infinitystorage.gui.grid.sorting.GridSortingQuantity;
import infinitystorage.gui.grid.stack.ClientStackFluid;
import infinitystorage.gui.grid.stack.ClientStackItem;
import infinitystorage.gui.grid.stack.IClientStack;
import infinitystorage.gui.sidebutton.*;
import infinitystorage.integration.jei.IntegrationJEI;
import infinitystorage.integration.jei.InfinityStorageJEIPlugin;
...
public class GuiGrid extends GuiBase {
    ...
    public GuiGrid(ContainerGrid container, IGrid grid) {
        ...
        scrollbar = new Scrollbar(174, 20, 12, (grid.getType() == EnumGridType.CRAFTING || grid.getType() == EnumGridType.PATTERN || grid.getType() == EnumGridType.FLUID) ? 70 : 88);
    }
    ...
    public void drawForeground(int mouseX, int mouseY) {
        ...
        if (isOverCreatePattern(mouseX, mouseY)) {
            drawTooltip(mouseX, mouseY, t("gui.infinitystorage:grid.pattern_create"));
        }
    }
    ...
    public void mouseClicked(int mouseX, int mouseY, int clickedButton) throws IOException {
        ...
        if (clickedCreatePattern) {
            ...
            InfinityStorage.INSTANCE.network.sendToServer(new MessageGridPatternCreate(gridPos.getX(), gridPos.getY(), gridPos.getZ()));
        } else if (grid.isConnected()) {
            if (clickedClear) {
                InfinityStorage.INSTANCE.network.sendToServer(new MessageGridCraftingClear((TileGrid) grid));
            }
            ...
            if (isOverSlotArea(mouseX - guiLeft, mouseY - guiTop) && held != null && (clickedButton == 0 || clickedButton == 1)) {
                InfinityStorage.INSTANCE.network.sendToServer(grid.getType() == EnumGridType.FLUID ? new MessageGridFluidInsertHeld() : new MessageGridItemInsertHeld(clickedButton == 1));
            }
            ...
            if (isOverSlotWithItem()) {
                if (grid.getType() != EnumGridType.FLUID && (held == null || (held != null && clickedButton == 2))) {
                    ...
                    if (stack.isCraftable() && (stack.getQuantity() == 0 || (GuiScreen.isShiftKeyDown() && GuiScreen.isCtrlKeyDown()))) {
                        ...
                        InfinityStorage.INSTANCE.network.sendToServer(new MessageGridItemPull(stack.getHash(), flags));
                    }
                } else if (grid.getType() == EnumGridType.FLUID && held == null) {
                    InfinityStorage.INSTANCE.network.sendToServer(new MessageGridFluidPull(STACKS.get(slotNumber).getHash(), GuiScreen.isShiftKeyDown()));
                }
            }
        }
    ...
    private void updateJEI() {
        if (IntegrationJEI.isLoaded() && (grid.getSearchBoxMode() == TileGrid.SEARCH_BOX_MODE_JEI_SYNCHRONIZED || grid.getSearchBoxMode() == TileGrid.SEARCH_BOX_MODE_JEI_SYNCHRONIZED_AUTOSELECTED)) {
            InfinityStorageJEIPlugin.INSTANCE.getRuntime().getItemListOverlay().setFilterText(searchField.getText());
        }
    }
