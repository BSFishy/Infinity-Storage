...
import infinitystorage.item.ItemBlockController;
...
public class BlockController extends BlockBase {
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (!world.isRemote) {
            player.openGui(InfinityStorage.INSTANCE, InfinityStorageGui.CONTROLLER, world, pos.getX(), pos.getY(), pos.getZ());
        }
    ...
    public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
        ...
        ItemStack stack = new ItemStack(InfinityStorageBlocks.CONTROLLER, 1, InfinityStorageBlocks.CONTROLLER.getMetaFromState(state));
