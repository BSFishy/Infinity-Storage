...
import infinitystorage.tile.TileFluidStorage;

import java.util.List;

public class ItemBlockFluidStorage extends ItemBlockBase {
    public ItemBlockFluidStorage() {
        super(InfinityStorageBlocks.FLUID_STORAGE, InfinityStorageBlocks.FLUID_STORAGE.getPlacementType(), true);
    }
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List<String> tooltip, boolean advanced) {
        ...
        if (type != null && isValid(stack)) {
            ...
            if (type == EnumFluidStorageType.TYPE_CREATIVE) {
                tooltip.add(I18n.format("misc.infinitystorage:storage.stored", FluidStorageNBT.getStoredFromNBT(tag)));
            } else {
                tooltip.add(I18n.format("misc.infinitystorage:storage.stored_capacity", FluidStorageNBT.getStoredFromNBT(tag), type.getCapacity()));
            }
        }
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(ItemStack stack, World world, EntityPlayer player, EnumHand hand) {
        ...
        if (type != null && stack.stackSize == 1 && isValid(stack) && FluidStorageNBT.getStoredFromNBT(stack.getTagCompound().getCompoundTag(TileFluidStorage.NBT_STORAGE)) == 0 && stack.getMetadata() != ItemFluidStorageDisk.TYPE_CREATIVE && !world.isRemote && player.isSneaking()) {
            ItemStack storagePart = new ItemStack(InfinityStorageItems.FLUID_STORAGE_PART, 1, stack.getMetadata());
