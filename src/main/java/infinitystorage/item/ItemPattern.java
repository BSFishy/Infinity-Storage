...
import infinitystorage.apiimpl.autocrafting.CraftingPattern;
...
public class ItemPattern extends ItemBase implements ICraftingPatternProvider {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List<String> tooltip, boolean advanced) {
        ...
        if (pattern.isValid()) {
            if (GuiScreen.isShiftKeyDown() || isProcessing(stack)) {
                tooltip.add(TextFormatting.YELLOW + I18n.format("misc.infinitystorage:pattern.inputs") + TextFormatting.RESET);

                combineItems(tooltip, true, Iterables.toArray(pattern.getInputs(), ItemStack.class));

                tooltip.add(TextFormatting.YELLOW + I18n.format("misc.infinitystorage:pattern.outputs") + TextFormatting.RESET);
            }

            combineItems(tooltip, true, Iterables.toArray(pattern.getOutputs(), ItemStack.class));

            if(pattern.isOredicted()){
                tooltip.add(TextFormatting.BLUE + I18n.format("misc.infinitystorage:pattern.oredict"));
            }
        } else {
            tooltip.add(TextFormatting.RED + I18n.format("misc.infinitystorage:pattern.invalid") + TextFormatting.RESET);
        }
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(ItemStack stack, World world, EntityPlayer player, EnumHand hand) {
        if (!world.isRemote && player.isSneaking()) {
            return new ActionResult<>(EnumActionResult.SUCCESS, new ItemStack(InfinityStorageItems.PATTERN));
        }
