...
public class ItemStorageDisk extends ItemBase {
    ...
    private void applyDebugDiskData(ItemStack stack) {
        if (debugDiskTag == null) {
            ...
            while (it.hasNext()) {
                ...
                if (item != InfinityStorageItems.STORAGE_DISK) {
    ...
    public void addInformation(ItemStack disk, EntityPlayer player, List<String> tooltip, boolean advanced) {
        if (ItemStorageNBT.isValid(disk)) {
            ...
            if (capacity == -1) {
                tooltip.add(I18n.format("misc.infinitystorage:storage.stored", ItemStorageNBT.getStoredFromNBT(disk.getTagCompound())));
            } else {
                tooltip.add(I18n.format("misc.infinitystorage:storage.stored_capacity", ItemStorageNBT.getStoredFromNBT(disk.getTagCompound()), capacity));
            }
        }
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(ItemStack disk, World world, EntityPlayer player, EnumHand hand) {
        if (!world.isRemote && player.isSneaking() && ItemStorageNBT.isValid(disk) && ItemStorageNBT.getStoredFromNBT(disk.getTagCompound()) == 0 && disk.getMetadata() != TYPE_CREATIVE) {
            ItemStack storagePart = new ItemStack(InfinityStorageItems.STORAGE_PART, 1, disk.getMetadata());
            ...
            return new ActionResult<>(EnumActionResult.SUCCESS, new ItemStack(InfinityStorageItems.STORAGE_HOUSING));
        }
