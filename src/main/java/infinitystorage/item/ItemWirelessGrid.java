...
import infinitystorage.integration.tesla.WirelessGridEnergyTesla;
...
public class ItemWirelessGrid extends ItemEnergyContainer implements ISpecialElectricItem, IElectricItemManager {
    ...
    public ItemWirelessGrid() {
        ...
        setRegistryName(InfinityStorage.ID, "wireless_grid");
        setMaxDamage(3200);
        setMaxStackSize(1);
        setHasSubtypes(true);
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List<String> tooltip, boolean advanced) {
        if (stack.getItemDamage() != TYPE_CREATIVE) {
            tooltip.add(I18n.format("misc.infinitystorage:energy_stored", getEnergyStored(stack), getMaxEnergyStored(stack)));
        }

        if (isValid(stack)) {
            tooltip.add(I18n.format("misc.infinitystorage:wireless_grid.tooltip", getX(stack), getY(stack), getZ(stack)));
        }
    }
        ...
        if (!world.isRemote && isValid(stack)) {
            ...
            if (controllerWorld != null && ((controller = controllerWorld.getTileEntity(new BlockPos(getX(stack), getY(stack), getZ(stack)))) instanceof TileController)) {
                if (((TileController) controller).getWirelessGridHandler().onOpen(player, controllerWorld, hand)) {
                    ...
                    player.addChatComponentMessage(new TextComponentTranslation("misc.infinitystorage:wireless_grid.out_of_range"));
                }
            } else {
                player.addChatComponentMessage(new TextComponentTranslation("misc.infinitystorage:wireless_grid.not_found"));
            }
        }
    ...
    public String getUnlocalizedName() {
        return "item." + InfinityStorage.ID + ":wireless_grid";
    }
