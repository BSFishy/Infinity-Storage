...
import infinitystorage.item.ItemNetworkCard;
...
public class TileNetworkTransmitter extends TileNode {
    ...
    private ItemHandlerBasic networkCard = new ItemHandlerBasic(1, this, new ItemValidatorBasic(InfinityStorageItems.NETWORK_CARD)) {
        @Override
        protected void onContentsChanged(int slot) {
            super.onContentsChanged(slot);
    ...
    public int getEnergyUsage() {
        return InfinityStorage.INSTANCE.networkTransmitterUsage + (isSameDimension() ? (int) Math.ceil(InfinityStorage.INSTANCE.networkTransmitterPerBlockUsage * getDistance()) : 0) + upgrades.getEnergyUsage();
    }
