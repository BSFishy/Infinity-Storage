...
import infinitystorage.apiimpl.network.NetworkNodeGraph;
import infinitystorage.apiimpl.network.WirelessGridHandler;
import infinitystorage.apiimpl.network.grid.FluidGridHandler;
import infinitystorage.apiimpl.network.grid.ItemGridHandler;
import infinitystorage.apiimpl.storage.fluid.FluidUtils;
import infinitystorage.apiimpl.storage.fluid.GroupedFluidStorage;
import infinitystorage.apiimpl.storage.item.GroupedItemStorage;
import infinitystorage.block.BlockController;
...
import infinitystorage.integration.ic2.IntegrationIC2;
import infinitystorage.integration.tesla.ControllerEnergyTesla;
import infinitystorage.integration.tesla.IntegrationTesla;
import infinitystorage.network.MessageGridFluidDelta;
import infinitystorage.network.MessageGridFluidUpdate;
import infinitystorage.network.MessageGridItemDelta;
import infinitystorage.network.MessageGridItemUpdate;
...
public class TileController extends TileBase implements INetworkMaster, IEnergyReceiver, IRedstoneConfigurable {
    ...
    public static final TileDataParameter<List<ClientNode>> NODES = new TileDataParameter<>(InfinityStorageSerializers.CLIENT_NODE_SERIALIZER, new ArrayList<>(), new ITileDataProducer<List<ClientNode>, TileController>() {
        @Override
    ...
    public void update() {
        if (!worldObj.isRemote) {
            ...
            if (getType() == EnumControllerType.NORMAL) {
                if (!InfinityStorage.INSTANCE.controllerUsesEnergy) {
    ...
    public void sendItemStorageToClient(EntityPlayerMP player) {
        InfinityStorage.INSTANCE.network.sendTo(new MessageGridItemUpdate(this), player);
    }
    ...
    public void sendFluidStorageToClient(EntityPlayerMP player) {
        InfinityStorage.INSTANCE.network.sendTo(new MessageGridFluidUpdate(this), player);
    }
    ...
    public static ICraftingTask readCraftingTask(World world, NBTTagCompound tag) {
        ...
        if (stack != null && stack.getItem() instanceof ICraftingPatternProvider) {
            ...
            if (container instanceof ICraftingPatternContainer) {
                ...
                ICraftingTaskFactory factory = InfinityStorageAPI.instance().getCraftingTaskRegistry().getFactory(tag.getString(CraftingTask.NBT_PATTERN_TYPE));
    ...
    public int getEnergyUsage() {
        int usage = InfinityStorage.INSTANCE.controllerBaseUsage;
    ...
    public EnumControllerType getType() {
        if (type == null && worldObj.getBlockState(pos).getBlock() == InfinityStorageBlocks.CONTROLLER) {
